...
def build_parser(possible_tokens):
    ...
    @pg.production('expression : I IZ IDENTIFIER input_args MKAY')
    def func_call(p):
        name = p[2].getstr()
        arguments = p[3]
        return FunctionCallExpression(children=[name, arguments])
        
    @pg.production('input_args : ')
    def input_args_empty(p):
        return []
        
    @pg.production('input_args : optional_an YR expression input_args')
    def input_args_literal(p):
        return [p[2]] + p[3]
        
    @pg.production('statement : func_header newlines code_block func_footer')
    def function(p):
        name = p[0][0]
        arguments = p[0][1]
        return_type = p[3]
        block = p[2]
        return FunctionDeclaration(children=[name, arguments, return_type, block])
    
    @pg.production('func_header : HOW IZ I IDENTIFIER arguments MKAY')
    def func_header(p):
        return p[3].getstr(), p[4]
        
    @pg.production('arguments : ')
    def argument_empty(p):
        return []
        
    @pg.production('arguments : optional_an YR IDENTIFIER literal_type_declaration arguments')
    def argument_literal(p):
        name = p[2].getstr()
        expr_type = p[3]
        argument = [(name, expr_type)]
        arguments = argument + p[4]
        return arguments
        
    @pg.production('arguments : optional_an YR IDENTIFIER array_type_declaration arguments')
    def argument_array(p):
        name = p[2].getstr()
        expr_type = p[3]
        argument = [(name, expr_type)]
        arguments = argument + p[4]
    ...
    @pg.production('func_footer : IF U SAY SO literal_type_declaration')
    def litaral_func_footer(p):
        return p[4]
        
    @pg.production('func_footer : IF U SAY SO array_type_declaration')
    def array_func_footer(p):
        return p[4]
    
    @pg.production('statement : FOUND YR expression')
    def return_statment(p):
        return FoundStatement(children=[p[2]])
    ...
    @pg.production('statement : WTF QUESTION_MARK expression newlines comparison_blocks optional_omgwtf OIC')
    def switch_statement(p):
        conditional = p[2]
        comparisons = p[4]
        default = p[5]
        return WTFStatement(children=[conditional, comparisons, default])
        
    @pg.production('comparison_blocks : ')
    def empty_comp(p):
        pass
    
    @pg.production('comparison_blocks : comparison_block comparison_blocks')
    def comparisons(p):
        if p[1]:
            return [p[0]] + p[1]
        return [p[0]]
    
    @pg.production('comparison_block : OMG expression newlines code_block')
    def comparison_block(p):
        return p[1], p[3]
    
    @pg.production('optional_omgwtf : OMGWTF newlines code_block')
    @pg.production('optional_omgwtf : ')
    def default_comparison(p):
        if not p:
            return None
        return p[2]
