...
class VisibleStatement(ASTNode):
    ...
    def compile(self, symbol_table, compiled_code):
        ...
        for child in self.children:
            ...
            if child_entry.expr_type in PrimitiveType:
...
class FunctionDeclaration(ASTNode):
    def compile(self, symbol_table, compiled_code):
        name = self.children[0]
        arguments = self.children[1]
        return_type = self.children[2]
        block = self.children[3]
        init_backup = symbol_table.var_count
        symbol_table.backup.append(init_backup)
        
        label = symbol_table.get_unique_label(root='function')
        symbol_table.declare_function(name, return_type, arguments)
        symbol_table.push_call_stack(name)
        symbol_table.increment_scope()
        
        if len(symbol_table.declared_variables.maps) > 3:
            raise FunctionCallError()
        
        compiled_code.append([f"# Start of function definition for {name}"])
        compiled_code.append([f"JUMP end_of__{name}__{label}"])
        compiled_code.append([f"_{name}__function:"])
        
        for argument in reversed(arguments):
            argument_entry = symbol_table.declare_variable(argument[0], argument[1])
            compiled_code.append([f"POP {argument_entry}"])
        block.compile(symbol_table, compiled_code)
        
        #for i in range(symbol_table.var_count):
        #    symbol_table.backup.append(i+init_backup)
            
        #print(symbol_table.backup)
        
        compiled_code.append([f"end_of__{name}__{label}:"])
        compiled_code.append([f"# End of function definition for {name}"])
        
        symbol_table.decrement_scope()
        symbol_table.pop_call_stack()
...
class FunctionCallExpression(ASTNode):
    def compile(self, symbol_table, compiled_code):
        ...
        function = symbol_table.declared_functions[name]
        ...
        label = symbol_table.get_unique_label(root=f"function_call_to_{name}")
        
        compiled_code.append([f"# Compiling Call To {name}"])
        
        if len(symbol_table.call_stack) > 0:
            if symbol_table.read_call_stack() == name:
                for i in range(symbol_table.var_count):
                    if i > symbol_table.backup[0]:
                        if i not in symbol_table.pushpop:
                            if i not in symbol_table.backup:
                                symbol_table.backup.append(i)
                for i in symbol_table.backup:
                    compiled_code.append([f"PUSH s{i}"])
                    
        compiled_code.append([f"PUSH {label}"])
        for i in range(len(inputs)):
            inp_entry = inputs[i].compile(symbol_table, compiled_code)
            if inp_entry.expr_type != arguments[i][1]:
                raise FunctionCallError()
            compiled_code.append([f"PUSH {inp_entry}"])
        compiled_code.append([f"JUMP _{name}__function"])
        compiled_code.append([f"{label}:"])
        
        pop = symbol_table.get_entry(expr_type=return_type)
        compiled_code.append([f"POP {pop}"])
        if len(symbol_table.call_stack) > 0:
            if symbol_table.read_call_stack() == name:
                for i in reversed(symbol_table.backup):
                    compiled_code.append([f"POP s{i}"])
        return pop
        

class FoundStatement(ASTNode):
    """
    A statement that represents a return statment in a function
    """
    def compile(self, symbol_table, compiled_code):
        name = symbol_table.read_call_stack()
        function = symbol_table.declared_functions[name]
        return_type = function[0]
        compiled_code.append(["# Compiling Found Statement"])
        push = self.children[0].compile(symbol_table, compiled_code)
        pop = symbol_table.get_entry(expr_type=PrimitiveType.NUMBR)
        symbol_table.pushpop.append(push.address)
        symbol_table.pushpop.append(pop.address)
        if push.expr_type != return_type:
            raise FunctionCallError()
        compiled_code.append([f"POP {pop}"])
        compiled_code.append([f"PUSH {push}"])
        compiled_code.append([f"JUMP {pop}"])
...
class WTFStatement(ASTNode):
    ...
    def compile(self, symbol_table, compiled_code):
        ...
        cases = self.children[1]
        if self.children[2]:
            cases.append([self.children[2]])
        
        compiled_code.append(['# Compiling WTF Statement'])
        expr_entry = variable.compile(symbol_table, compiled_code)
