...
class SymbolTable:
    ...
    def declare_function(self, name, return_type, args):
        if name in self.declared_functions:
            raise RedeclarationError(f'{name} has already been declared!')
        self.declared_functions[name] = (return_type, args)
        result = FunctionEntry(name, return_type, args)
        return result
    ...
    def push_call_stack(self, label):
        self.call_stack.append(label)
    
    def read_call_stack(self):
        if not self.call_stack:
            raise UndeclaredError('Attempting to read empty call stack')
...
class FunctionEntry:
    ...
    def __repr__(self):
        return f'{self.name}{self.return_type}{self.args}'
